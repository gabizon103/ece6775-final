...
int main(int argc, char** argv) {
    ...
    cl_mem_ext_ptr_t pe_data8_ext;
    pe_data8_ext.flags = HBM[8];
    pe_data8_ext.obj = matrix_split[8].data();
    pe_data8_ext.param = 0;
    err = CL_SUCCESS;

    cl::Buffer pe_data8_buf(
        context,
        CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        BFS_SIZE * sizeof(int),
        &pe_data8_ext,
    ...
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to allocate device memory for pe_data8, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }

    cl_mem_ext_ptr_t pe_data9_ext;
    pe_data9_ext.flags = HBM[9];
    pe_data9_ext.obj = matrix_split[9].data();
    pe_data9_ext.param = 0;
    err = CL_SUCCESS;

    cl::Buffer pe_data9_buf(
        context,
        CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        BFS_SIZE * sizeof(int),
        &pe_data9_ext,
    ...
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to allocate device memory for pe_data9, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }

    cl_mem_ext_ptr_t pe_data10_ext;
    pe_data10_ext.flags = HBM[10];
    pe_data10_ext.obj = matrix_split[10].data();
    pe_data10_ext.param = 0;
    err = CL_SUCCESS;

    cl::Buffer pe_data10_buf(
        context,
        CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        BFS_SIZE * sizeof(int),
        &pe_data10_ext,
    ...
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to allocate device memory for pe_data10, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }

    cl_mem_ext_ptr_t pe_data11_ext;
    pe_data11_ext.flags = HBM[11];
    pe_data11_ext.obj = matrix_split[11].data();
    pe_data11_ext.param = 0;
    err = CL_SUCCESS;

    cl::Buffer pe_data11_buf(
        context,
        CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        BFS_SIZE * sizeof(int),
        &pe_data11_ext,
    ...
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to allocate device memory for pe_data11, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }

    cl_mem_ext_ptr_t pe_data12_ext;
    pe_data12_ext.flags = HBM[12];
    pe_data12_ext.obj = matrix_split[12].data();
    pe_data12_ext.param = 0;
    err = CL_SUCCESS;

    cl::Buffer pe_data12_buf(
        context,
        CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        BFS_SIZE * sizeof(int),
        &pe_data12_ext,
    ...
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to allocate device memory for pe_data12, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }

    cl_mem_ext_ptr_t pe_data13_ext;
    pe_data13_ext.flags = HBM[13];
    pe_data13_ext.obj = matrix_split[13].data();
    pe_data13_ext.param = 0;
    err = CL_SUCCESS;

    cl::Buffer pe_data13_buf(
        context,
        CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        BFS_SIZE * sizeof(int),
        &pe_data13_ext,
    ...
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to allocate device memory for pe_data13, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }

    cl_mem_ext_ptr_t pe_data14_ext;
    pe_data14_ext.flags = HBM[14];
    pe_data14_ext.obj = matrix_split[14].data();
    pe_data14_ext.param = 0;
    err = CL_SUCCESS;

    cl::Buffer pe_data14_buf(
        context,
        CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        BFS_SIZE * sizeof(int),
        &pe_data14_ext,
    ...
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to allocate device memory for pe_data14, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }

    cl_mem_ext_ptr_t pe_data15_ext;
    pe_data15_ext.flags = HBM[15];
    pe_data15_ext.obj = matrix_split[15].data();
    pe_data15_ext.param = 0;
    err = CL_SUCCESS;

    cl::Buffer pe_data15_buf(
        context,
        CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        BFS_SIZE * sizeof(int),
        &pe_data15_ext,
    ...
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to allocate device memory for pe_data15, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }

    cl_mem_ext_ptr_t pe_counter_ext;
    pe_counter_ext.flags = HBM[16];
    ...
    final_frontier_ext.flags = HBM[17];
    ...
    err = command_q.enqueueMigrateMemObjects(
        ...
        pe_data5_buf, pe_data6_buf, pe_data7_buf, pe_data8_buf, pe_data9_buf, 
        pe_data10_buf, pe_data11_buf, pe_data12_buf, pe_data13_buf, pe_data14_buf, 
        pe_data15_buf, pe_counter_buf},
    ...
    err = kernel.setArg(8, pe_data8_buf);
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to set kernel argument 8, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }
    err = kernel.setArg(9, pe_data9_buf);
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to set kernel argument 9, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }
    err = kernel.setArg(10, pe_data10_buf);
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to set kernel argument 10, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }
    err = kernel.setArg(11, pe_data11_buf);
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to set kernel argument 11, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }
    err = kernel.setArg(12, pe_data12_buf);
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to set kernel argument 12, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }
    err = kernel.setArg(13, pe_data13_buf);
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to set kernel argument 13, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }
    err = kernel.setArg(14, pe_data14_buf);
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to set kernel argument 14, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }
    err = kernel.setArg(15, pe_data15_buf);
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to set kernel argument 15, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }

    err = kernel.setArg(16, pe_counter_buf);
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to set kernel argument 8, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }
    err = kernel.setArg(17, final_frontier_buf);
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to set kernel argument 9, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
    }
    err = kernel.setArg(18, num_hops);
    if (err != CL_SUCCESS) {
        std::cerr << "[ERROR]: Failed to set kernel argument 10, exit!" << std::endl;
        std::cerr << "         Error code: " << err << std::endl;
        return 1;
